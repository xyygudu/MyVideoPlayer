...
(mp4, mkv — passed through as the output path's extension), and a quality
preset dropdown ("高质量" / "均衡" / "快速") that maps to a fixed
`{crf, preset}` pair per option (e.g. 高质量→crf 18/slow, 均衡→crf 23/medium,
快速→crf 28/fast). `EncodeParams::codec_name` SHALL be fixed to `"libx264"`
(video) and `"aac"` (audio) — no codec choice is exposed since only these
two are validated.

...
VideoFrame 的内部实现（`Impl`）SHALL 使用 `AVFramePtr` 管理 FFmpeg 帧引用（而非裸 `AVFrame*` + 手写析构），确保 RAII 一致性。

VideoFrame SHALL 由 Decoder 在解码循环中直接构建（PTS 已换算为秒），而非由显示层延迟创建。

VideoFrame 析构时 SHALL 自动释放其持有的内部缓冲区引用（通过 AVFramePtr 析构实现）。
...
- **THEN** AVFramePtr 析构触发 av_frame_free，引用计数减一

#### Scenario: VideoFrame Impl uses AVFramePtr
- **WHEN** 查看 VideoFrame::Impl 内部结构
- **THEN** 使用 AVFramePtr 成员（非裸 AVFrame*），无手写析构函数
...
AudioFrame 的内部实现（`Impl`）SHALL 使用 `AVFramePtr` 管理 FFmpeg 帧引用。

AudioFrame SHALL 由 Decoder 在解码循环中直接构建。
...
AVFramePtr SHALL 在以下场景中使用：
1. VideoFrame::Impl / AudioFrame::Impl 内部持有帧引用
2. Decoder 解码循环中作为临时接收缓冲区（`avcodec_receive_frame` 输出）

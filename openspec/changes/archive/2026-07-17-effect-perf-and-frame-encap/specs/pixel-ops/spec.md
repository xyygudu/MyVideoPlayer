...
- `RemapInterleavedPlane(src, dst, width, height, fill, mapping)`：对 NV12 交织色度平面做一次逆映射同时写入 U 和 V，避免 InverseMap 重复计算
- `TryPermutePlane(src, dst, width, height, comp_stride, comp_offset, params) -> bool`：若变换为纯排列重排（旋转 0/90/180/270° + 任意翻转 + scale=1.0 + translate=0），用整数下标重排完成拷贝并返回 true；否则返回 false，调用方应 fallback 到 `RemapPlane`
...
- **WHEN** 逆映射坐标落在源平面之外
- **THEN** 对应目标像素填 `fill` 值，不访问源内存

#### Scenario: TryPermutePlane 命中纯排列场景
- **WHEN** 参数为 rotate_deg=90, flip_h=true, scale=1.0, translate=0
- **THEN** TryPermutePlane 返回 true，目标平面为正确的 90 度顺时针+水平镜像像素排列，不执行任何浮点运算

#### Scenario: TryPermutePlane 未命中时返回 false
- **WHEN** 参数 rotate_deg=37.5（非直角）
- **THEN** TryPermutePlane 返回 false，源和目标平面均未被修改

### Requirement: pixel_ops 不依赖任何 graph 或帧封装类型
`pixel_ops.h` SHALL 仅依赖 `<cstdint>`，不引用 `AVFrame`、`MediaFrame`、`MediaBuffer`、`INode`、`MediaGraph` 等任何 FFmpeg 或项目 graph 层类型。

#### Scenario: pixel_ops.h 可被独立编译
- **WHEN** 在仅包含 `<cstdint>` 的环境中编译 `pixel_ops.h`
- **THEN** 编译通过，不产生任何未定义标识符错误

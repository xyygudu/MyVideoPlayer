...
### Requirement: DemuxNode::StreamInfoMap() 暴露内部状态
**Reason**: `StreamInfoMap()` 仅服务于 `BuildGraph` 获取流信息，现由 `SourceProbe::Probe()` 替代。
**Migration**: 调用方改用 `SourceProbe::Probe(filepath)` 获取 `SourceInfo`，不再依赖 `DemuxNode` 暴露内部 map。

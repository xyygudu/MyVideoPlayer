...
The previous HWAccelContext design — a decode-only god-class hardcoded to
D3D11VA, created by the pipeline builder before graph construction — is
superseded. Device lifecycle now lives in the `gpu-device` capability, the
get_format callback lives in DecoderNode, and whether hardware decode is
used is decided during negotiation, not before graph construction (the
architectural violation that caused the 2026-05 implementation to be
removed).

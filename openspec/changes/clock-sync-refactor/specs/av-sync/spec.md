...
系统 SHALL 在 AudioMaster 模式下，视频渲染循环中将视频帧的 PTS 与 `audio_clock_.Get()` 对比，决定显示时机。同步阈值 SHALL 使用命名常量 `kSyncThreshold` 和 `kDropThreshold`。

#### Scenario: Video frame PTS <= audio clock
- **WHEN** AudioMaster 模式，视频帧 PTS 与 audio_clock 差值在 [-kDropThreshold, kSyncThreshold] 范围内
- **THEN** 系统立即显示该帧

#### Scenario: Video frame PTS > audio clock beyond threshold
- **WHEN** AudioMaster 模式，视频帧 PTS - audio_clock > kSyncThreshold
- **THEN** 系统等待差值时间（上限 kMaxSleepSeconds）后再显示该帧

#### Scenario: Video frame is too late
- **WHEN** AudioMaster 模式，audio_clock - 视频帧 PTS > kDropThreshold
